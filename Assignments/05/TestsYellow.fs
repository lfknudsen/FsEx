...
open Interpreter.EvalYellow
open Interpreter.StateYellow
...
let assign3 () =
    let assign = Seq(Declare "result",
                       If(TT, Assign("result", Num 2), Assign("result", Num 2)))
    Assert.Equal<Result<int, error>>(Ok 2, () |> mkState |> stmntEval assign |> bind (getVar "result"))

[<Fact>]
let assign4 () =
    let assign = Seq(Declare "result",
                       If(FF, Assign("result", Num 2), Assign("result", Num 2)))
    Assert.Equal<Result<int, error>>(Ok 2, mkState () |> stmntEval assign |> bind (getVar "result"))
...
let TestFactorial1 () =
    ...
    match res with
    | Ok v -> System.Console.WriteLine("TestFactorial1() : OK! Fac 5: " + string v)
    | Error e -> System.Console.WriteLine("TestFactorial1() : Err :( Fac 5: " + string e)
...
let TestFactorial2 () =
    ...
    match res with
    | Ok v -> System.Console.WriteLine("TestFactorial2() : Fac 5: " + string v)
    | Error e -> System.Console.WriteLine("TestFactorial2() : Fac 5: " + string e)
    Assert.Equal<Result<int, error>>(Ok 120, res)
